...
NeedlemanWunsch <- function(S,R,delta, decisionf=max, Peor=-Inf)
{
	...
	for (i in 2:m)
	{
		trellis[i,1] <- trellis[i-1, 1] + delta[u[i], "-"]    #COMPLETAR
		backi[i,1] <- i-1 
		backj[i,1] <- 1
	}
	...
	for (j in 2:n)
	{
		trellis[1,j] <- trellis[1, j-1] + delta["-", v[j]]    #COMPLETAR
		backi[1,j] <- 1 
		backj[1,j] <- j-1
	}
	...
	for (i in 2:m)
	{
		...
		{
			#borrado de u[i]
			costes["B"] <- trellis[i-1, j] + delta[u[i], "-"]  #COMPLETAR
			
			#insercion de v[j]
			costes["I"] <- trellis[i, j-1] + delta["-", v[j]]  #COMPLETAR

			#sustitucion o match u[i]==v[j]
			costes["MoS"] <- trellis[i-1, j-1] + delta[u[i], v[j]]   #COMPLETAR
...
SmithWaterman <- function(S,R,delta, decisionf=max,Peor=-Inf)
{
	...
	#ojo!!! los vectores comienzan en 1 por lo queestán desplazados para que concuerden las formulas con la teoria

	#vector temporal de costes de las operaciones, volvemos a ponerlos a Inf o -Inf
	costes <- c(Peor,Peor,Peor,0)#COMPLETAR
...
#definicion de la matriz de puntuación
m <- 1
s <- -1
b <- -1 #borrado 0
i <- -1 #insercion 0
...
t45sw <- SmithWaterman(seq4,seq5,delta)z
print(seq5)
print(seq5)
